...
UPD: сортировка была некорректна, т.к. приводила к сортировке в алфавитном порядке. Изменено на to_char(s.sale_date, 'ID') - конвертация даты в дни недели в ISO-формате.
*/
...
--6
/*
Через джоины собираем таблицу из таблицы фактов и двух справочников, а после SELECT'ом выбираем три поля: 1) дату в формате YYYY-MM, 2) число уникальных покупателей через COUNT(DISTINCT),
3) выручка как цена за единицу товара на количество проданных штук с округлением до целых.
*/
...
--7
/*
Оконными функциями ищем "первые вхождения" промоционных товаров и соотв. дату. Обертка в CTE для удобства написания запроса :)
*/
