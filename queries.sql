...
--7
/*
OLD:Оконными функциями ищем "первые вхождения" промоционных
товаров и соотв. дату. Обертка в CTE для удобства написания
запроса :)

UPD: исправлено без помощи оконных функций, а только через корректную
фильтрацию, INNER JOIN'ы (чтобы выкидывать unmatched значения из
более правых таблиц), а также через DISTINCT ON с корректной
сортировкой - последняя должна соответствовать "left-most
column in distinct on clause" - pSQL doc.
*/
